...
Como podemos ver na tabela \ref{tab:inpdesc} existem partidos que existem partidos que estão consideravelmente acima da média no INP: o máximo de 2018, por exemplo, 0.73. Tanto o desvio padrão, quanto o coeficiente de variação tambeém são evidências de que alguns partidos são consideravelmente diferentes dos demais. O boxplot é interessante aqui já que ele aparenta indicar um padrão específico de variação entre as eleições. Elas conhecidem com pleitos envolvendo incumbentes na disputa: a variação no grau de nacionalização é menor. Tanto as caixas são menores, quanto os mínimos são sempre maiores que nos anos com eleição sem incumbente.
